...
public class Naive implements Player{
	...
	public int getMove(int[] board) throws IOException{
		...
		System.out.println("wr: "+wr+" br: "+br);
		if(canWin(board)){
			System.out.println("Attempting to pick a winning move.");
			if(wr == 1){
				System.out.println("Failed, picking random move.");
				return searchMove(board,"r");
			}
			return searchMove(board,"w");
		}else if(canBlock(board)){
			System.out.println("Attempting to pick a blocking move.");
			if(br == 1){
				System.out.println("Failed, picking random move.");
				return searchMove(board,"r");
			}
			...
			System.out.println("Attempting to pick a strategy move.");
			if(sr == 1){
				System.out.println("Failed, picking random move.");
				return searchMove(board,"r");
			}
